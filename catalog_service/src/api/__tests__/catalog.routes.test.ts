...
const mockRequest = () => {
  return {
    ...
    stock: faker.number.bigInt({ min: 10, max: 100 }),
...
describe("Catalog Routes", () => {
  describe("POST /products", () => {
    ...
    test("should repsonse with validation error 400", async () => {
      const requestBody = mockRequest();
    ...
    test("should repsonse with internal error code 500", async () => {
      const requestBody = mockRequest();
  ...
  describe("GET /produts/?limit=0&offset=0", () => {
    test("should return a range of products based on limit and offset", async () => {
      ...
      const response = await request(app)
        .patch(`/products/limit=${randomLimit}&offset=0`)
        .set("Accept", "application/json");
      expect(response.status).toBe(200);
